...
class App extends Component {
  ...
  async onSwipe(direction, listingId, sessionId) {
    ...
    if (direction === 'left') {
      await axios.delete(`http://server-svc:5000/listings/delete/${listingId}`);
    } else {
      await axios.post('http://server-svc:5000/listings/recordSwipe', {
        id: listingId,
  ...
  async handleClick(listingId) {
    ...
    await axios.post('http://server-svc:5000/listings/updateLike', {
      id: listingId,
    });
  }
  ...
  async componentWillMount() {
    const response = await axios.get(`http://server-svc:5000/listings`);
